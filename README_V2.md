...
Le projet couvre l'ensemble du cycle de vie d'un projet de Data Science :

- collecte des données,
- ingestion et stockage,
- prétraitement des signaux audio,
- construction de jeux de données d'entraînement,
- expérimentation et suivi des modèles d'apprentissage machine,
- industrialisation du modèle retenu,
- déploiement d'une API REST et d'une interface web.

L'objectif est de produire automatiquement plusieurs artefacts exploitables à partir d'un fichier audio au format **WAV** :

- transcription MIDI,
- piano roll (SVG et PNG),
- partition musicale (SVG et PDF).
...
Ce projet vise deux objectifs complémentaires.

Le premier consiste à développer une plateforme reproductible permettant de construire des jeux de données de qualité en assurant la gouverance et le lineage des données et d'expérimenter plusieurs architectures d'apprentissage machine en garantissant le suivi des expérimentations grâce à MLflow.

Le second consiste à industrialiser le modèle retenu afin de proposer un service de transcription automatique accessible via une API REST et une interface web, dans un environnement reproductible et entièrement automatisé.

Pour répondre à ces objectifs, le projet est scindé en deux parties distinctes :

- une **plateforme Data** permettant de construire les jeux de données et d'expérimenter différents modèles d'apprentissage machine,
- une **couche applicative** permettant de mettre un modèle en production grâce à une API REST développée avec **FastAPI** et une interface utilisateur développée avec **Streamlit**.

L'ensemble est entièrement conteneurisé avec **Docker**, automatisé par **GitHub Actions** et déployé sur **Hugging Face Spaces**.
...
| Élément | Emplacement |
| :- | :- |
| Documentation (livrables 1, 5 et 6) | `livrables/` |
| Code ETL | `audio_midi/src/` |
| Notebooks (livrables 2, 3 et 4) | `audio_midi/notebooks/` |
| API | `api/backend/` |
| Interface | `api/frontend/` |
| Tests | `api/tests/` |
...
- One-vs-Rest + HistGradientBoosting,
- MLP,
- CNN + MLP,
- RCNN.
...
Le modèle retenu est ensuite exporté manuellement au format **TensorFlow** `.keras` et intégré à la couche applicative ([api/backend/artifacts/](./api/backend/artifacts/)) pour son déploiement.
...
Une documentation OpenAPI est disponible à l'adresse <http://localhost:8000/docs>.
...
La couche applicative est entièrement conteneurisée avec **Docker** et peut être exécutée :

en développement local via docker compose ;
automatiquement sur Hugging Face Spaces.
