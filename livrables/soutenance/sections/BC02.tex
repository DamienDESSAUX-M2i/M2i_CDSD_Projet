...
\begin{frame}{Analyse du dataset GuitarSet - Polyphonie}
	\small
...
\begin{frame}{Analyse d'un dataset Frame-Wise}
	\begin{columns}[T]
		...
		\small
		6 valeurs propres supérieures à 1. Elles expliques au moins 92\% de la variance.
	\end{columns}
\end{frame}
...
\begin{frame}{Analyse d'un dataset Frame-Wise}
	\begin{columns}[T]
		...
		\small
		63 frames contiennent plus de 6 activations simultanée, non aligné avec la physique de l'instrument. En moyenne, il y a 1.81 ($\pm$ 1.55) notes actives par frame. Environ 40\% des frames contiennent aux moins deux notes simultanées.
	\end{columns}
\end{frame}